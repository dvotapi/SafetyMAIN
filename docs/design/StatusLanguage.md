...
| Status                       | Russian UI label                  | Token         | Icon key         | Domains                |
| ---------------------------- | --------------------------------- | ------------- | ---------------- | ---------------------- |
| Draft                        | Черновик                          | `draft`       | `file-pen`       | Hazard, RA, Control, … |
| Under Review                 | На рассмотрении                   | `review`      | `eye`            | RA                     |
| Approved                     | Утверждено                        | `approved`    | `badge-check`    | RA                     |
| Rejected                     | Отклонено                         | `rejected`    | `badge-x`        | RA                     |
| Planned                      | Запланировано                     | `planned`     | `calendar-clock` | Control, Inspection    |
| Active                       | Действует                         | `active`      | `circle-check`   | Hazard                 |
| In Implementation            | Внедряется                        | `inProgress`  | `loader`         | Control                |
| Implemented                  | Внедрено                          | `implemented` | `package-check`  | Control                |
| Verified Effective           | Подтверждена эффективной          | `effective`   | `shield-check`   | Control                |
| Verified Partially Effective | Подтверждена частично эффективной | `partial`     | `shield-alert`   | Control                |
| Verified Ineffective         | Подтверждена неэффективной        | `ineffective` | `shield-x`       | Control                |
| Overdue                      | Просрочено                        | `overdue`     | `clock-alert`    | Control review, tasks  |
| Suspended                    | Приостановлено                    | `suspended`   | `pause`          | Control                |
| Archived                     | Архив                             | `archived`    | `archive`        | All                    |
| Cancelled                    | Отменено                          | `cancelled`   | `ban`            | Control, RA            |
| Superseded                   | Замещено                          | `superseded`  | `replace`        | Control, RA            |
