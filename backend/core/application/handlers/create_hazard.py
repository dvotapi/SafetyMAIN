...
from backend.core.application.commands.hazard_lifecycle import CreateHazardCommand
...
from backend.core.domain.exceptions.hazard import DuplicateHazardCode
...
class CreateHazardHandler:
    ...
    def handle(self, command: CreateHazardCommand) -> Hazard:
        ...
        def operation() -> Hazard:
            ...
            now = self._clock.now()
            identified_at = command.identified_at or now
            return Hazard.create(
                ...
                identified_by=command.actor_id,
                created_at=now,
            )

        def success_spec(hazard: Hazard) -> AuditRecordSpec:
            self._unit_of_work.hazards.add(hazard)
